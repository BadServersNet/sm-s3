...
static S3ClientObject *ReadClient(IPluginContext *pContext, cell_t handle)
{
	...
	const HandleError error =
		handlesys->ReadHandle(handle, g_ClientType, &security, reinterpret_cast<void **>(&client));
...
static S3ResponseObject *ReadResponse(IPluginContext *pContext, cell_t handle)
{
	...
	const HandleError error =
		handlesys->ReadHandle(handle, g_ResponseType, &security, reinterpret_cast<void **>(&response));
...
static S3ObjectListObject *ReadObjectList(IPluginContext *pContext, cell_t handle)
{
	...
	const HandleError error =
		handlesys->ReadHandle(handle, g_ObjectListType, &security, reinterpret_cast<void **>(&list));
...
static IChangeableForward *
CreateCallbackForward(IPluginContext *pContext, cell_t function, int paramCount, const ParamType *types)
{
...
static cell_t Submit(
	IPluginContext *pContext,
	S3ClientObject *client,
	JobSpec spec,
	cell_t completedFunction,
	cell_t data,
	cell_t progressFunction,
	bool listCallback
)
{
	IChangeableForward *completed = listCallback ? CreateListForward(pContext, completedFunction)
												 : CreateCompletedForward(pContext, completedFunction);
...
static cell_t Native_S3Response_GetHeader(IPluginContext *pContext, const cell_t *params)
{
	...
	const std::string name = s3::ToLower(ReadString(pContext, params[2]));
...
static const s3::ListedObject *ReadListedObject(IPluginContext *pContext, const cell_t *params)
{
	...
	if (index < 0 || static_cast<size_t>(index) >= list->objects.size())
	{
		pContext->ThrowNativeError(
			"Index %d is out of bounds (size %d)", index, static_cast<int>(list->objects.size())
		);
